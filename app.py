...
# ✅ Tabs définis AVANT toute utilisation
tabs = st.tabs([
    "Vue générale",
...
with tabs[2]:
    ...
    ab_columns = [col for col in tests_semaine.columns if col.lower() not in ['semaine']]
...
with tabs[3]:
    ...
    other_ab_columns = [col for col in other_ab.columns if col.lower() not in ['week']]
    ...
    fig = px.line(other_ab, x='Week', y=selected_other_ab, title=f"% Résistance - {selected_other_ab}", markers=True)
    alertes = other_ab[other_ab['Alarme']]
    fig.add_scatter(x=alertes['Week'], y=alertes[selected_other_ab], mode='markers', marker=dict(color='red', size=10), name='Alarme')
    st.plotly_chart(fig)
